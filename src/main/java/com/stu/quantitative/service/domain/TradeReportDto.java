...
public class TradeReportDto {
    ...
    public void exchange(LocalDate date, int investCode, int stockCode, int direction, double price, double quantity) {
    ...
    // 总仓日志
    public void clearing(LocalDate date,SettlementRecordDto settlementRecordDto){
        if(null!=currentTable && currentTable.getDate().equals(date)){
            //  当日有交易记录，则记录相关清算信息
            this.currentTable.clearing(settlementRecordDto);
        }
    }

    public void report(){
        this.tables.forEach(TradeTableDto::report);
    }
}
