...
public class S0 {
    //  预期仓位
    private final B0 balanceAccount;
    ...
    public S0(B0 balanceAccount, StockEntity stock) {
        this.stock = stock;
    ...
    /*
        ...
        3. 执行交易
     */
    public int endGameExecute(Td0 record) {
        ...
        // 2.1 计算当前股票的历史波动率
        this.technicalAnalysis.add(this.kLine.getClose());
    ...
    public int backTradeExechte(Td0 record) {
        this.kLine = record.getPrices().get(this.stock.getTicker()).orElse(null);
        if (this.kLine == null) {
            log.warn("{} 没有K线数据", this.stock.getTicker());
            return 0;
    ...
    // 盘后清算
    public void clearing(Td0 record, int direction) {
        if (this.kLine == null) {
            return ;
        }
        if (direction > 0) {
    ...
    // 是否已上市
    public boolean tradeable(Td0 record) {
...
//        this.direction = 0 或 2 全部跳过
    ...
    private void addTradeToLog(Td0 record) {
        if (this.direction == 0) {
//            如果当天无真实交易，直接退出
            return;
        }
        Td1 tradeRecordDto = new Td1(this.stock.getName(), this.direction, this.exchangePrice, this.exchangeQuantity, this.hv, this.balanceAccount.getCallShare(), this.balanceAccount.getPutShare(), this.callTrend, this.putTrend, this.callRate, this.putRate);
        record.addTrade(tradeRecordDto);
        // 计算完毕后立即归位
        this.direction = 0;
    }

    public void addShareToLog(Td0 record) {
        ...
        Sr0 shareRecordDto = new Sr0(this.getStock().getName(), this.kLine.getClose(), this.shareQuantity, this.shareAmount, this.balanceAccount.getPosition(), this.call, this.put, callQuantity, putQuantity);
