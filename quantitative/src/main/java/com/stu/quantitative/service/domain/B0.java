...
public class B0 {
    ...
    private final List<S0> stockAccounts;
    ...
    public B0(String investName, double expectedShareRate, List<StockEntity> stocks){
        this.expectedShareRate = expectedShareRate;
        this.coefficient = 1 - this.expectedShareRate * 2;
        this.investName = investName;
        this.stocks =stocks;
        this.stockAccounts = stocks.stream().map(it->new S0(this,it)).toList();
    }

    // 残局交易
    public void endGameExecute(Td0 record){
        ...
        // 2. 执行盘后清算任务
        // 2.1 计算当天仓位总数
        this.shareAmount = this.stockAccounts.stream().mapToDouble(S0::getShareAmount).sum();
        // 2.2 计算仓位占比
    ...
    // 回测交易
    public void backTradeExechte(Td0 record){
        this.currentDate = record.getDate();
        // 1.执行盘中交易任务
        ...
        this.shareAmount = this.stockAccounts.stream().filter(it->it.tradeable(record))
                .mapToDouble(S0::getShareAmount).sum();
        // 2.2 计算仓位占比
        this.position = this.shareAmount / this.policy.totalAmount();
        // 2.3 对每个股票进行盘后清算
    ...
    public void injectPolicy(P0 policy){
        this.policy = policy;
